...
В примерах ниже это разделение на test_is_palindrome и test_is_palindrome_is_not_palindrome. Такие тесты понятнее и проще читать.
Пример test_is_palindrome с ожидаемым результатом читается сложнеие, потому что в нем тестируются два различных поведения.
"""
